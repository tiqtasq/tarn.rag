...
from examples.part_ii import _runner
...
async def test_runner_ingests_corpus_and_renders_without_error(tmp_path):
    async with TarnRag(_hash_settings(tmp_path)) as tarn:
        _runner.banner("P2-test", shows="the runner", fails="nothing", fixed_next="n/a")
        await _runner.ingest_corpus(tarn, corpus("corpus-2"))
...
async def test_runner_retrieval_verdict_matches_actual_retrieval(tmp_path):
    ...
    async with TarnRag(_hash_settings(tmp_path)) as tarn:
        ...
        verdict = await _runner.show_retrieval(
            tarn, "storage tank inspection", gold="quokka", top_k=8, label="probe"
        )
        assert verdict is expected
        assert await _runner.show_retrieval(tarn, "anything", gold=None) is None  # no gold -> no verdict
