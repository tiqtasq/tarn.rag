...
- **Embedding model — configurable, with a recommended default.** Default: a small sentence-embedding model exported to ONNX, e.g. **`bge-small-en-v1.5`** (384-dim) for retrieval quality, or **`all-MiniLM-L6-v2`** (384-dim, symmetric, no prefix) for maximum config simplicity. The model is **swappable** via the `Embedder` interface; the **only** hard rule is that retrieval's embedding pipeline (model + prefixes + pooling + normalization) is **byte-for-byte the pipeline ingestion used**, as recorded in the index (§8.3).
