...
| §5.4 Dense + Sparse retrievers (incl. the permitted-chunk **pre-filter**) | ✅ built (`DenseRetriever` / `SparseRetriever`). The permitted-chunk filter is now applied **inside** each retriever: `dense_knn`/`sparse_search` take a `ChunkFilter` and over-fetch (×4) to backfill past disallowed chunks (`_overfetch`), so `top_k` holds for tight scopes (finding 1.2). On Postgres the filtered dense path raises `ivfflat.probes` for full recall. *(In-`vec0`-metadata pre-filtering per §8.3 is still not used — the over-fetch fallback covers it.)* |
| §5.5 RRF fusion | ✅ built (`RRFFuser`) with the mandatory `(score desc, chunk_id asc)` tie-break (finding 1.1) — identical inputs yield identical orderings (the §9 cross-port parity contract). |
| §5.6 license + scope filtering | ✅ built — `available`, `ai_grounding_allowed` (for `GENERATION_GROUNDING`), method **scope**, and the per-purpose **`LicensePolicy`** (license-class-by-purpose; `DefaultLicensePolicy` = the §5.6 table, `third_party_copyrighted` never permitted; configurable via the `LICENSE_POLICY` spec). |
