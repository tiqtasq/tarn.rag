...
cited steps; ``SingleHopReasoner``), the ``EvidenceAssembler`` (cited indices → provenance-bearing
``Citation``s; ``ProvenanceAssembler``), and the optional ``GroundingChecker`` (verify each claim against
its cited evidence; ``HeuristicGroundingChecker`` / ``LLMGroundingChecker``). Each self-registers with the
